...
Entries are grouped by component (Protocol, Client, Server, Dev Packages, E2E) per release.
...
> [`glsp-server-node`](https://github.com/eclipse-glsp/glsp-server-node) (Server),
> [`glsp`](https://github.com/eclipse-glsp/glsp) (Dev Packages) and
> [`glsp-playwright`](https://github.com/eclipse-glsp/glsp-playwright) (E2E).
> The changelogs of `glsp-client`, `glsp-server-node` and `glsp` have been merged here; release and pull request links still point to the original repositories.
> The protocol was part of `glsp-client`, so pre-consolidation protocol changes are recorded under Client as `[protocol]` entries.
> The `glsp-playwright` changelog was not merged, so the E2E group has no entries below; its history remains in the original repository.
