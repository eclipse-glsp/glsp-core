...
- The 5 GLSP repos are: `glsp-core`, `glsp-theia-integration`, `glsp-vscode-integration`, `glsp-eclipse-integration`, `glsp-server`. `glsp-core` consolidates the former `glsp` (dev packages), `glsp-client` and `glsp-server-node` repositories and publishes all `@eclipse-glsp` npm packages the integration repos build against — so it is the first repo that must go to next.
