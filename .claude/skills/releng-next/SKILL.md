...
- **no-push** (optional): if the user asks to _not push_ / run locally / dry-run, pass `--no-push` to `prepare next`. The version bump and `nightly-<version>` commit are made in the temp clone but **nothing is pushed and no PR is created**. Default: **push enabled**.
- **draft** (optional): if the user asks for _draft_ PRs, pass `--draft` to `prepare next` so the PRs are created as drafts. Only meaningful when pushing. Default: **non-draft**.
...
- Always use the **local** CLI built from this repo: `pnpm exec glsp …` (never `npx @eclipse-glsp/cli`). The maintainer wants the behavior of the code as it currently stands.
- `releng prepare next` and `releng version` accept `-r/--repoDir <dir>` and `cd` into it internally, so stay in `glsp-core` and point `-r` at each temp clone — do not run from inside each repo.
...
- The 6 GLSP repos are: `glsp-core`, `glsp-theia-integration`, `glsp-vscode-integration`, `glsp-eclipse-integration`, `glsp-server`, `glsp-playwright`. `glsp-core` consolidates the former `glsp` (dev packages), `glsp-client` and `glsp-server-node` repositories and publishes all `@eclipse-glsp` npm packages the integration repos build against — so it is the first repo that must go to next.
...
Run from the `glsp-core` repository root.
...
2. Build the local CLI so `dist/cli.js` is current:
    ...
    Confirm `pnpm exec glsp --help` works before continuing.
...
4. Clone the target repos fresh into `WORKDIR`, from the real upstream (no fork, so `origin = eclipse-glsp` and pushes/PRs target upstream):
    ...
    ```bash
    # full run (default): all 6 repos
    pnpm exec glsp repo clone --preset all -d "$WORKDIR"

    # partial run: only the repos the user named (repo clone takes positional names)
    pnpm exec glsp repo clone <repo> [<repo> …] -d "$WORKDIR"
    ```
...
**Processing order:** always process `glsp-eclipse-integration` **last** (after all other targeted repos), because its build depends on `glsp-server`'s next P2 being published — see _Dependency ordering_ in Conventions.
...
```bash
pnpm exec glsp releng prepare next -r "$WORKDIR/<repo>" [--no-push] [--draft]
```
...
Skip this phase entirely (and tell the user why) if **no-push** was used (no PRs exist) or if **draft** was used (drafts are left for manual review) — see _Input interactions_.
...
2. **Retry prepare next** for eclipse-integration (same flags as the original run, push enabled):
    ...
    ```bash
    pnpm exec glsp releng prepare next -r "$WORKDIR/glsp-eclipse-integration"
    ```
