...
`CHANGELOG.md` is a combined changelog: every release section groups its entries by component (`### Protocol`, `### Client`, `### Server`, `### Dev Packages`, `### E2E`), and each component has its own `#### Changes` / `#### Potentially Breaking Changes` subsections. Entries below the "predate the consolidation" disclaimer belong to the former `glsp-client`, `glsp-server-node` and `glsp` repositories — **never edit them or their links** (the protocol was part of `glsp-client` back then, so those sections record protocol changes under Client).

Execute these phases in order:

1. **Data Gathering** — find last release, fetch PRs, assign components, classify, generate entries
...
Record as `TAG_DATE`. The tag history predates the monorepo consolidation, so the tag date is used only as a cutoff for the PR query — sanity-check that it is plausible (not in the future, not older than the previous changelog section) before relying on it.

### Step 1.2: Extract category tags from existing CHANGELOG.md

Read `CHANGELOG.md` and extract all unique `[tag]` patterns from changelog entries (pattern: `- [tag] ...`). Build a deduplicated vocabulary list preserving original casing, and note which component subgroup each tag typically appears under — `[diagram]`, `[routing]` are client tags, `[gmodel]`, `[operation]`, `[layout]` are server tags, `[cli]`, `[eslint-config]`, `[ts-config]` are dev-package tags. The `[protocol]` tag marks protocol entries recorded under Client before the protocol became its own component; inside the `### Protocol` section prefer finer tags (e.g. `[action]`, `[di]`, `[uuid]`).
...
Use the date portion only from `TAG_DATE` (e.g. `2026-06-01`).
...
### Step 1.5: Assign a component

Determine the component from the files the PR touched:

```bash
gh pr view <N> --json files --jq '.files[].path'
```

| Changed paths                                                                        | Component        |
| ------------------------------------------------------------------------------------ | ---------------- |
| `packages/common/**`                                                                 | **Protocol**     |
| `packages/client/**`, `examples/workflow-glsp/**`, `examples/workflow-standalone/**` | **Client**       |
| `packages/server/**`, `examples/workflow-server*/**`                                 | **Server**       |
| `dev-packages/**`                                                                    | **Dev Packages** |
| `e2e/**`                                                                             | **E2E**          |

- A PR that touches several components produces **one entry per affected component**, each describing that component's part of the change (same PR link in each).
- A PR that only touches repository-wide infrastructure (root configs, `.github/**`, docs) has no natural component — flag it for user review in Phase 3 rather than guessing, unless it clearly belongs to one (e.g. a Node version bump that affects everything goes into every component, mirroring how `[node]` entries were recorded before).
...
Pick the most fitting tag from the vocabulary (Step 1.2) based on PR title and body, preferring tags already used within the target component. If no tag fits, mark as uncertain: `[???/best-guess]`.
...
```
- [tag] Description [#N](https://github.com/eclipse-glsp/glsp-core/pull/N)
```
...
- Entry prefix: `- ` (dash + one space)
- Sub-item indent: 4 spaces + `- `
...
- An active section has the `- active` suffix (e.g. `## v2.8.0 - active`)
- If an active section exists → merge new entries into it
- If the topmost section is a released version (no `- active` suffix) → create a new active section above it, directly below the intro line and above the disclaimer/`---` block

**Creating a new section:**

- Bump the minor version of `LAST_TAG` (e.g. `v2.7.0` → `v2.8.0`)
- Use the full four-component skeleton and fill in what applies:
...
- Keep all four component subgroups so contributors have a place to add entries, even when a component has none yet. Drop empty `#### Potentially Breaking Changes` headings only when the release is finalized, not while it is active.

**Merging into existing active section:**

- Check PR numbers against existing entries to avoid duplicates
- Append each entry to its component's subsection, into `#### Changes` or `#### Potentially Breaking Changes`
- Create missing subsections as needed, keeping the component order Protocol → Client → Server → Dev Packages → E2E

### Step 2.2: Write the updated CHANGELOG.md

Use the Edit tool to update `CHANGELOG.md` directly in the current working tree. Match existing formatting conventions and leave everything below the consolidation disclaimer untouched.
...
List any entries flagged as uncertain (component, category or breaking status) and ask the user to resolve them.
